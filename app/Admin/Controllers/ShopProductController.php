 ...
 */
    ...
    public function postCreate()
    {
        ...
        switch ($data['kind']) {
            case SC_PRODUCT_SINGLE: // product single
                $arrValidation = [
                    ...
                    'descriptions.*.general_details' => 'required|string',
                    'category' => 'required',
                ...
                $arrMsg = [
                    ...
                    'descriptions.*.general_details.required' => trans('validation.required', ['attribute' => trans('product.content')]),
        ...
        $dataInsert = [
            ...
            'outstanding' => (!empty($data['outstanding']) ? 1 : 0),
        ...
        foreach ($languages as $code => $value) {
            $dataDes[] = [
                ...
                'general_details' => $descriptions[$code]['general_details'] ?? '',
            ];
        }
 ...
 */
    public function edit($id)
    {
        ...
        $data = [
            ...
            'categories' => (new ShopCategory)->getTreeCategories(),
            'tags'=>ShopTag::where('status',1)->get(),
            'listTags'=> ShopProductTag::where('product_id', $product->id)->pluck('tag_id')->toArray(),
            'brands' => (new ShopBrand)->getList(),
            'suppliers' => (new ShopSupplier)->getList(),
            'products' => (new ShopProduct)->getData(),
            'taxs' => (new ShopTax)->getList(),
            'virtuals' => $this->virtuals,
            'kinds' => $this->kinds,
            'attributeGroup' => $this->attributeGroup,
        ...
        switch ($product['kind']) {
            case SC_PRODUCT_SINGLE: // product single
                ...
                $arrMsg = [
                    ...
                    'descriptions.*.content.required' => trans('validation.required', ['attribute' => trans('product.content')]),
                    'descriptions.*.general_details.required' => trans('validation.required', ['attribute' => trans('product.content')]),
                    'descriptions.*.info_details.required' => trans('validation.required', ['attribute' => trans('product.content')]),
                    'category.required' => trans('validation.required', ['attribute' => trans('product.category')]),
                    'sku.regex' => trans('product.sku_validate'),
                    'alias.regex' => trans('product.alias_validate'),
                ];
...
//Edit
        ...
        foreach ($data['descriptions'] as $code => $row) {
            $dataDes[] = [
                ...
                'content' => $row['content'] ?? '',
                'general_details' => $row['general_details'] ?? '',
            ];
        }
        ShopProductDescription::insert($dataDes);
