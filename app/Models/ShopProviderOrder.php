...
class ShopProviderOrder extends Model
{
    use ModelTrait;

    public $table = SC_DB_PREFIX.'provider_order';
    protected $guarded = [];
    ...
    public function getInforProvider()
    {
        return $this->hasOne(ShopProvider::class, 'id', 'supplier');
    }

    public function details()
    {
        return $this->hasMany(ShopProviderOrderDetail::class, 'order_id', 'id');
    }
    public function orderTotal()
    {
        return $this->hasMany(ShopProviderOrderTotal::class, 'order_id', 'id');
    }
    ...
    {
        return $this->hasMany(ShopProviderOrderHistory::class, 'order_id', 'id');
    }
...
//Scort
    ...
    {
        return ShopProviderOrderHistory::create($dataHistory);
    }
    ...
    {
        return ShopProviderOrderDetail::create($dataDetail);
    }
    ...
    public function getCountryInYearProvider() {
        return $this->selectRaw('country, count(id) as count')
            ->whereRaw('DATE(created_at) >=  DATE_SUB(DATE(NOW()), INTERVAL 12 MONTH)')
            ->whereRaw('provider_id = '. Admin::user()->provider_id)
            ->groupBy('country')->orderBy('count','desc')->get();
    }
    ...
    public function getSumOrderTotalInYearProvider() {
        return $this->selectRaw('DATE_FORMAT(created_at, "%Y-%m") AS ym,
        SUM(total/exchange_rate) AS total_amount')
            ->whereRaw('created_at >=  DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH), "%Y-%m-00")')
            ->whereRaw('provider_id = '. Admin::user()->provider_id)
    ...
    public function getSumOrderTotalInMonthProvider() {
        return $this->selectRaw('DATE_FORMAT(created_at, "%m-%d") AS md,
        SUM(total/exchange_rate) AS total_amount, count(id) AS total_order')
            ->whereRaw('created_at >=  DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), "%Y-%m-%d 00:00")')
            ->whereRaw('provider_id = '. Admin::user()->provider_id)
