...
class ShopCart extends GeneralController
{
    ...
    public function getCart()
    {
        ...
        foreach ($cart as $item){
            $arrCartItem[$i] = $item->id;
            $i++;
        }
//        dd(session('guestLocation'));
        if (auth()->user()){
            $location = auth()->user()->address1;
        }else{
            $location = session('guestLocation') ?? 'Thái Nguyên' ;
        }
        $providers = ShopProvider::where('province', $location)->orderBy('type', 'desc')->limit(5)->get();
        if ($providers->count() < 1 ){
            $providers = ShopProvider::orderBy('type', 'desc')->get();
        }
        foreach ($providers as $key => $provider){
            foreach ($arrCartItem as $item){
                if (!in_array($item, explode(',', $provider->products))){
                    unset($providers[$key]);
                }
            }
        }
        if ($providers->count() < 1 ){
            $providers = ShopProvider::orderBy('type', 'desc')->limit(3)->get();
        }

        $providers = ShopProvider::limit(5)->get();
//        dd($providers);
        //Shipping
        ...
        $shippingAddress = $addressDefaul ?? session('shippingAddress');
//        dd($shippingAddress);
...
//            'paymentMethod' => 'required',
        ...
        $messages = [
            ...
            'address3.required' => trans('validation.required',['attribute'=> 'Vui lòng nhập địa chỉ!']),
...
//        session(['shippingMethod' => request('shippingMethod')]);
    ...
    public function getCheckout()
    {
        ...
        $objects = ShopOrderTotal::getObjectOrderTotal();
        $dataTotal = ShopOrderTotal::processDataTotal($objects);
        $dataProvider = ShopProvider::find(session('shippingAddress.provider_id'));
    ...
    public function addOrder(Request $request)
    {
        ...
        $dataOrder['address3'] = $shippingAddress['address'] .", ". $shippingAddress['address3'];
        ...
        $dataOrder['provider_id'] = $shippingAddress['provider_id'];
        ...
        $notify['title'] = 'Có đơn hàng mới từ: '.$dataOrder['first_name'] ;
        $notify['url'] = '/sc_admin/order/detail/'.$createOrder['orderID'];
...
//        dd($pusher);
     ...
     * @return  [view]
     */
    public function getfiveprovider(Request $request){
//        dd($request);
        return $request;
//        if(!session('orderID')) {
//            return redirect()->route('home');
...
//        );
    ...
    public function getCatThep(){
        ...
        $products = (new ShopProduct)
            ...
            ->where($tableDescription . '.lang', sc_get_locale())->get();
        return view($this->templatePath.'.screen.cathep',array(
            'title' => 'Căt thép',
            'description' => '',
            'keyword' => '',
            'layout_page' => 'cat_thep',
            'products'=>$products
        ));
    }
    public function postCatThep(Request $request){
