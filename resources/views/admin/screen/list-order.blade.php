...
{{--          @endif--}}
     ...
     <div class="tableData">
         ...
         <ul class="nav nav-tabs" role="tablist">
             @foreach($orderStatus as $key => $status)
                 <li role="presentation" class="{{ ($loop->first) ? 'active' : '' }}">
                     <a href="#state{{ $key }}" aria-controls="{{ $status }}" role="tab" data-toggle="tab">{{ $status }}</a>
                 </li>
             @endforeach
         ...
         <div class="tab-content">
             @foreach($orderStatus as $key => $status)
                 <div role="tabpanel" class="tab-pane fade {{ ($loop->first) ? 'active in' : '' }}" id="state{{ $key }}">
                                        ...
                                        @foreach($dataTable[$status] as $data)
                                            @if($data->status == 1)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('admin_order.show_order', ['id' => $data->id]) }}"><span
                                                                    title="Xem" type="button"
                                                                    class="btn btn-flat btn-primary"><i
                                                                        class="fa fa-eye"></i></span></a>
                                                        <a href="{{ route('admin_order.detail', ['id' => $data->id]) }}"><span
                             ...
                             <table class="table table-hover dataTable" id="dataTable">
                                 ...
                                 @foreach ($dataTr as $keyRow => $tr)
                                     <tr>
                                         @if (!empty($removeList))
                                             <td>
                                                 <input type="checkbox" class="input grid-row-checkbox" data-id="{{ $tr['id']??'' }}">
                                             </td>
                                         @endif
                                         @foreach ($tr as $key => $trtd)
                                             <td>{!! $trtd !!}</td>
