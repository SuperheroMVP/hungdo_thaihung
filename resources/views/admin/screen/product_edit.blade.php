                    ...
                    <div class="fields-group">
                        ...
                        <div
                            ...
                            @if ($product->kind == SC_PRODUCT_SINGLE)
                                <div
                                    class="form-group {{ $errors->has('descriptions.'.$code.'.general_details') ? ' has-error' : '' }}">
                                    <label for="{{ $code }}__content"
                                           class="col-sm-2 col-form-label">Thông tin chung</label>
                                    <div class="col-sm-8">
                                        <textarea id="{{ $code }}__content" class="editor"
                                  name="descriptions[{{ $code }}][general_details]">
                            {!! old('descriptions.'.$code.'.general_details',($descriptions[$code]['general_details']??'')) !!}</textarea>
                                        @if ($errors->has('descriptions.'.$code.'.general_details'))
                                            <span class="help-block">
                                            <i class="fa fa-info-circle"></i>
                                            {{ $errors->first('descriptions.'.$code.'.general_details') }}
                                        </span>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="form-group  kind kind0 kind1 {{ $errors->has('tags') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control input-sm category select2" multiple="multiple"
                                    ...
                                    @foreach ($tags as $k => $v)
                                        <option value="{{ $v->id }}"
                                                {{ (count($listTags) && in_array($v->id, $listTags))?'selected':'' }}>{{ $v->title }}
                                        </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('tags'))
...
@endif
                        ...
                        <div class="form-group  {{ $errors->has('relation_id') ? ' has-error' : '' }}">
                            @php
                            $listRelation = [];
                            $relation_id = old('relation_id', explode(',', $product->relation_id));
                            if(is_array($relation_id)){
                                foreach($relation_id as $value){
                                    $listRelation[] = (int)$value;
                                }
                            }
                            @endphp
                            <label for="supplier_id" class="col-sm-2 col-form-label">SP Liên quan</label>
                            <div class="col-sm-8">
                                <select class="form-control input-sm relation_id select2" multiple="multiple"
                                    data-placeholder="Chọn sản phẩm liên quan" style="width: 100%;"
                                    name="relation_id[]">
                                    <option value=""></option>
                                    @foreach ($products as $k => $v)
                                    <option value="{{ $v->id }}"
                                        {{ (count($listRelation) && in_array($v->id, $listRelation))?'selected':'' }}>{{ $v->name }}
                                    </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('relation_id'))
                                <span class="help-block">
                                    <i class="fa fa-info-circle"></i> {{ $errors->first('relation_id') }}
                                </span>
                                @endif
                            </div>
                        </div>
...
@endif
                        @if ($product->kind == SC_PRODUCT_SINGLE || $product->kind == SC_PRODUCT_BUILD)
                        {{-- unit --}}
                        <div class="form-group  kind kind0 kind1  {{ $errors->has('unit') ? ' has-error' : '' }}">
                            <label for="cost" class="col-sm-2 col-form-label">Đơn vị tính</label>
                            <div class="col-sm-8">
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-pencil fa-fw"></i></span>
                                    <input type="text" id="unit" name="unit"
                                           value="{!! old('unit',$product->unit) !!}" class="form-control input-sm cost"
                                           placeholder="" />
                                </div>
                                @if ($errors->has('unit'))
                                    <span class="help-block">
                                    <i class="fa fa-info-circle"></i> {{ $errors->first('unit') }}
                                </span>
                                @endif
                            </div>
                        </div>
...
@endif
                        ...
                        <div class="form-group  ">
                            <label for="status" class="col-sm-2 col-form-label">Nổi bật</label>
                            <div class="col-sm-8">
                                @if (old())
                                    <input class="input" type="checkbox" name="outstanding" {{ old('outstanding',$product['outstanding'])?'checked':''}}>
                                @else
                                    @if($product['outstanding']==1)
                                        <input class="input" type="checkbox" name="outstanding" checked>
                                    @else
                                        <input class="input" type="checkbox" name="outstanding" >
                                    @endif
                                @endif
