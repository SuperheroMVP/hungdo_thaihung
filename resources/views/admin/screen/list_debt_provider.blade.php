                        ...
                        <div class="modal-content">
                            ...
                            <form action="{{route('admin_provider_debt.create')}}" method="post">
                                        ...
                                        <table class="table">
                                            ...
                                            @foreach($debt_provider_list as $key => $debt_provider)
                                                <tr>
                                                    ...
                                                    <td><input type="number" class="input_value_debt" value="" id="debt_{{$debt_provider->id}}"
                                                               name="debt_provider[{{$debt_provider->id}}]"></td>
                                                </tr>
                                            @endforeach
                                            </tbody>
                                        </table>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="submit" class="btn btn-primary btn_submit_debt_provider" disabled>Lưu</button>
                                </div>
                            </form>
                        </div>
                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group row">
                                            <label for="time" class="col-sm-3 col-form-label">Thời gian</label>
                                            <div class="col-sm-7">
                                                <input type="date" class="form-control" id="view_time"
                                                       name="date"
                                                       value="">
                                            </div>
                                        </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group row">
                                            <label for="time" class="col-sm-3 col-form-label">Thời gian</label>
                                            <div class="col-sm-7">
                                                <input type="date" class="form-control" id="dh_view_time"
                                                       name="date"
                                                       value="" readonly>
                                            </div>
                                        </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
                ...
                if (thu_tu_khach != null && thu_tu_khach > 0) {
                    if (!(thu_tu_khach <= value_debt)) {
                        ...
                        for (let i = 0; i < debt_provider_list.length; i++) {
                            ...
                            $('.btn_submit_debt_provider').removeAttr('disabled');
                        }
                    } else {
            ...
            $(".input_value_debt").keyup(function ()  {
                ...
                if (tong_thu_don_hang == thu_tu_khach){
                    ...
                    $('.btn_submit_debt_provider').attr("disabled", true);
                }
            });

            //code js de number format data input
        ...
        $(".view_debt").on('click', function () {
            ...
            let url = "{{url('/sc_admin/debt/show')}}/" + id;
            $.ajax({
                type: 'GET',
                url: url,
                success: function (data) {
                    console.log(data['debt_provider_description'])
                    ...
                    data['debt_provider_description'].forEach(function (data) {
                        // console.log(data)
                        var newRowContent = "<tr><th>" + data['provider_order_id'] + "</th><td>" + data['date'] + "</td><td>" + new Intl.NumberFormat('de-DE', {
                            style: 'currency',
                            currency: 'VND'
        ...
        $(".view_order").on('click', function () {
            let id = $(this).attr("data-id");
            let url = "{{url('/sc_admin/debt/show_order')}}/" + id;
            $.ajax({
                type: 'GET',
                url: url,
                success: function (data) {
                    ...
                    $('#dh_view_show_debt').val(new Intl.NumberFormat('de-DE', {
                        style: 'currency',
                        currency: 'VND'
                    }).format(data['provider_order']['total']));
                    $('#dh_view_thu_tu_khach').val(new Intl.NumberFormat('de-DE', {
                        style: 'currency',
                        currency: 'VND'
                    }).format(data['provider_order']['received']));
                    $('#dh_view_no_sau').val(new Intl.NumberFormat('de-DE', {
                        style: 'currency',
                        currency: 'VND'
                    }).format(data['provider_order']['balance']));
                    // $('#view_time').val(data['debt_provider']['time']);
                    var time = new Date(data['provider_order']['created_at']);
                    time.setMinutes(time.getMinutes() + 60);
                    document.getElementById("dh_view_time").valueAsNumber = time.getTime();
                    $('#dh_view_note').val(data['provider_order']['note']);
                }
            });
        });
