    ...
    <div class="form_content {{ (old('check_red'))?'in':'' }}" id="collapseExample">
        ...
        @if (sc_config('customer_lastname'))
            ...
            <div class="form-group{{ $errors->has('reg_last_name') ? ' has-error' : '' }}">
                <input type="text"
                       ...
                       name="reg_last_name" placeholder="{{ trans('account.last_name') }}"
        ...
        @if (sc_config('customer_postcode'))
            <div class="form-group{{ $errors->has('reg_postcode') ? ' has-error' : '' }}">
                <input type="text"
                       ...
                       name="reg_postcode" placeholder="{{ trans('account.postcode') }}"
                       value="{{ old('reg_postcode') }}">
        ...
        @if (sc_config('customer_address2'))
            <div class="form-group{{ $errors->has('reg_address1') ? ' has-error' : '' }}">
                <select class="select2 form-control" style="width: 100%" id="province"
                        name="reg_address1">
                    <option value="{{ (old('address1'))?old('address1'): ''}}">{{ (old('address1'))?old('address1'): 'Chọn Tỉnh/Thành phố'}}</option>
                </select>
            ...
            <div class="form-group{{ $errors->has('reg_address2') ? ' has-error' : '' }}">
                <select class="select2 form-control" style="width: 100%" id="district"
                        name="reg_address2">
                    <option value="{{ (old('address2'))?old('address2'): ''}}">{{ (old('address2'))?old('address2'): 'Chọn Quận/Huyện'}}</option>
                </select>
            ...
            <div class="form-group{{ $errors->has('reg_address3') ? ' has-error' : '' }}">
                <select class="select2 form-control" style="width: 100%" id="ward"
                        name="reg_address3">
                    <option value="{{ (old('address3'))?old('address3'): ''}}">{{ (old('address3'))?old('address3'): 'Chọn Xã/Phường'}}</option>
                </select>
                @if($errors->has('address3'))
                    <span class="help-block">{{ $errors->first('address3') }}</span>
                @endif
            </div>
                <div class="form-group{{ $errors->has('reg_address') ? ' has-error' : '' }}">
                    <input type="text"
                           class="is_required validate account_input form-control {{ ($errors->has('reg_address'))?"input-error":"" }}"
                           name="reg_address" placeholder="{{ trans('account.address') }}"
                           value="{{ old('reg_address') }}">
        ...
        @else
            <div class="form-group{{ $errors->has('reg_address1') ? ' has-error' : '' }}">
                <input type="text"
                       ...
                       name="reg_address1" placeholder="{{ trans('account.address') }}"
                       value="{{ old('reg_address1') }}">
        ...
        @if (sc_config('customer_sex'))
            <div class="form-group{{ $errors->has('reg_sex') ? ' has-error' : '' }}">
                <label class="validate account_input {{ ($errors->has('reg_sex'))?"input-error":"" }}">{{ trans('account.sex') }}
                    : </label>
                <label class="radio-inline"><input value="0" type="radio"
                                                   name="reg_sex" {{ (old('reg_sex') == 0)?'checked':'' }}> {{ trans('account.sex_women') }}
                </label>
                <label class="radio-inline"><input value="1" type="radio"
                                                   name="reg_sex" {{ (old('reg_sex') == 1)?'checked':'' }}> {{ trans('account.sex_men') }}
                </label>
